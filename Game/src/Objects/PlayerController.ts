import { mat4, PhysicsObject, PhysicsScene, Renderer3D, Scene, vec2, vec3 } from "praccen-web-engine";
...
export default class PlayerController {
    ...
    if (Input.keys["ARROWLEFT"]) {
      mouseDiff[0] -= 5;
		...
		Input.updateGamepad();
		if (vec2.squaredLength(vec2.fromValues(Input.joystickLeftDirection[0], Input.joystickLeftDirection[1])) > 0.001) {
			vec3.scaleAndAdd(accVec, accVec, camera.getRight(), Input.joystickLeftDirection[0] * 2.0);
		...
		vec3.scaleAndAdd(this.physicsObject.force, vec3.create(), accVec, accelerationForce);

		// Jumping
		if ((Input.keys[" "] || Input.buttons.get("A")) && this.physicsObject.onGround) {
		...
		xzVelocity[1] = 0.0;
		vec3.scaleAndAdd(this.physicsObject.force, this.physicsObject.force, xzVelocity, -10.0);

		// Update camera
		this.game.camera.setPosition(vec3.fromValues(
